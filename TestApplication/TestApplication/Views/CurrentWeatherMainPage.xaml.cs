...
namespace TestApplication.Views
{
    ...
    public partial class CurrentWeatherMainPage : ContentPage
    {
        public CurrentWeatherMainPage()
        {
            InitializeComponent();
            GetWeatherInfo();
        }
        ...
        {
            var url = $"https://api.openweathermap.org/data/2.5/weather?q={Location}&appid=78ab003bd52f4adc10253a19a8b52f1d&units=metric";
        ...
        {
            var url = $"https://api.openweathermap.org/data/2.5/forecast?q={Location}&appid=78ab003bd52f4adc10253a19a8b52f1d&units=metric";
            var result = await ApiCaller.Get(url);
            ...
            if (result.Successful)
            {
                try
                {
                    ...
                    // setting all of the values into the UI of the xaml page
